...
public class BatchMigratorB {
    ...
    public static void main(String[] args) throws SQLException, ClassNotFoundException {
        ...
        // Define environment variable names for SQL statements
        String sourceSqlEnvVar = "SOURCE_SQL";
        String targetSqlEnvVar = "TARGET_SQL";

        // Get SQL statements from environment variables (default to empty strings)
        String sourceSql = Optional.ofNullable(System.getenv(sourceSqlEnvVar)).orElse("");
        String targetSql = Optional.ofNullable(System.getenv(targetSqlEnvVar)).orElse("");

        if (sourceSql.isEmpty() || targetSql.isEmpty()) {
            throw new IllegalArgumentException("Missing SQL statements in environment variables: " + sourceSqlEnvVar + ", " + targetSqlEnvVar);
        }
